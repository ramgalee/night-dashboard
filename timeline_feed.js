...
const 파일경로 = d => path.join(폴더, `${d}.json`);

function 읽기(d) {
  try { return JSON.parse(fs.readFileSync(파일경로(d), 'utf8')); } catch (e) { return { date: d, items: [] }; }
}
function 쓰기(d, v) {
  try {
    ...
    fs.renameSync(임시, 파일경로(d));
  } catch (e) {}
  // 오래된 날은 정리 (30일)
  try {
    const 목록 = fs.readdirSync(폴더).filter(f => /^\d{8}\.json$/.test(f)).sort();
    for (const f of 목록.slice(0, Math.max(0, 목록.length - 30))) fs.unlinkSync(path.join(폴더, f));
  } catch (e) {}
}
...
async function 글쓰기(본문, 직전, 시각) {
  const 키 = 키읽기();
  if (!키) throw new Error('ANTHROPIC_API_KEY 가 없습니다');

  const 머리 = `지금은 ${시각} 이다. 아래는 이 시각의 한국 증시 자료다.\n\n` + 본문 +
    (직전 ? `\n\n[직전 ${직전.time} 에 적은 글]\n${직전.title}\n${직전.body}` : '');
...
async function 한줄쌓기(강제) {
  ...
  try {
    ...
    // 같은 30분 칸에 이미 적었으면 건너뜁니다.
    // (09:00~09:29 · 09:30~09:59 … 식으로 하루 14칸)
    const 칸 = t => t.slice(0, 2) + (Number(t.slice(3, 5)) < 30 ? "A" : "B");
    if (직전 && 칸(직전.time) === 칸(시각)) return;

    const 본문 = await 자료모으기();
    if (!본문.trim()) return;

    const 글 = await 글쓰기(본문, 직전, 시각);
    파일.items.unshift({ time: 시각, at: new Date().toISOString(), ...글 });
    파일.date = d;
    쓰기(d, 파일);
    console.log(`[timeline] ${d} ${시각} · ${글.title}`);
  } catch (e) {
    console.log('[timeline] 실패:', String((e && e.message) || e));
  } finally { 도는중 = false; }
}

// 5분마다 확인해, 30분 칸이 바뀌었으면 한 줄 적습니다.
setInterval(() => { 한줄쌓기(false).catch(() => {}); }, 5 * 60 * 1000);
setTimeout(() => { 한줄쌓기(false).catch(() => {}); }, 20 * 1000);

module.exports = (app) => {
  app.get('/timeline', (req, res) => {
    const d = /^\d{8}$/.test(String(req.query.date || '')) ? String(req.query.date) : 오늘날짜();
    res.setHeader('Cache-Control', 'no-store');
    const 파일 = 읽기(d);
    res.json({ date: d, count: 파일.items.length, items: 파일.items, model: 쓰는모델 });
  });
